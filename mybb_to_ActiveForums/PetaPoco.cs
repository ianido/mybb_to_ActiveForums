					...
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						else
						{
							foreach (var colname in columns)
							{
                                ...
                                if (primaryKeyValue == null && primaryKeyValuePairs.ContainsKey(pc.ColumnName))
                                {
                                    primaryKeyValuePairs[pc.ColumnName] = pc.GetValue(poco);
                                    continue;
                                }
						...
						cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2}",
                                            _dbType.EscapeTableName(tableName), sb.ToString(), BuildPrimaryKeySql(primaryKeyValuePairs, ref index));

                        foreach (var pk in primaryKeyValuePairs){
                            PropertyInfo pkpi = pd.Columns[pk.Key].PropertyInfo;
						    AddParam(cmd, pk.Value, pkpi);
                        }
	...
	*/
	...
	public class TableInfo
	{
		...
		public static TableInfo FromPoco(Type t)
		{
            ...
            ti.AutoIncrement = ti.AutoIncrement ? !ti.PrimaryKey.Contains(',') : ti.AutoIncrement;
